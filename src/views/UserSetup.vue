            ...
            <ul class="mytab_list">
              <li>
                ...
                <a style="position: relative">
                  ...
                  <input class="opacity" @change="picChange" type="file" id="file0" name="file" title="点击上传图片" style="
                      z-index: 100;
                      cursor: pointer;
                      left: 0px;
                      top: -25px;
              ...
              <li>
                <i class="tit">我的昵称</i><router-link :to="{ name: 'setName' }" id="my_name">{{ nickName
                }}<em class="ml10">[修改]</em></router-link>
              </li>
              <li>
                <i class="tit">我的性别</i><router-link :to="{ name: 'setSex' }" id="my_sex">{{ sexText }}<em
                    class="ml10">[修改]</em></router-link>
              </li>
              <li>
                <i class="tit">我的密码</i><router-link :to="{ name: 'setPassword' }" id="my_password">修改密码</router-link>
              </li>
              <li>
                <i class="tit">当前状态</i><a @click="logout" href="javascript:void(0)">退出登录</a>
              </li>
            </ul>
          </div>
        </div>
...
import { getImgVerifyCode, uploadImg } from "@/api/resource";
import { getUserinfo, updateUserInfo } from "@/api/user";
import Header from "@/components/common/Header";
import Footer from "@/components/common/Footer";
import UserMenu from "@/components/user/Menu";
import { getToken, removeNickName, removeToken, removeUid } from "@/utils/auth";
export default {
  name: "userSetup",
  components: {
    Header,
  ...
  setup () {
    ...
    const picChange = async (event) => {
      ...
      if (file.type !== "image/jpeg") {
        ...
        event.target.value = ''; // 清空输入，允许重新选择
        return;
      }
